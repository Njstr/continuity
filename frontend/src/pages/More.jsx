    ...
    <div style={styles.screenPad}>
      <Section title="STARTUP PROFILE">
        {!editingProfile ? (
          ...
          <>
            <div style={styles.fieldGroup}><label style={styles.fieldLabel}>Startup name</label><input style={styles.selectInput} value={profileDraft.startupName || ""} onChange={(e) => setProfileDraft({ ...profileDraft, startupName: e.target.value })} /></div>
            <div style={styles.fieldGroup}><label style={styles.fieldLabel}>What are you building?</label><textarea style={{ ...styles.selectInput, minHeight: 80, resize: "vertical" }} value={profileDraft.oneLiner || ""} onChange={(e) => setProfileDraft({ ...profileDraft, oneLiner: e.target.value })} /></div>
            <div style={styles.fieldGroup}><label style={styles.fieldLabel}>Industry</label><select style={styles.selectInput} value={profileDraft.industry || ""} onChange={(e) => setProfileDraft({ ...profileDraft, industry: e.target.value })}><option value="">Not set yet</option>{INDUSTRIES.map((i) => <option key={i} value={i}>{i}</option>)}</select></div>
            <div style={styles.fieldGroup}><label style={styles.fieldLabel}>Stage</label><select style={styles.selectInput} value={profileDraft.stage || ""} onChange={(e) => setProfileDraft({ ...profileDraft, stage: e.target.value })}><option value="">Not set yet</option>{STARTUP_STAGES.map((i) => <option key={i} value={i}>{i}</option>)}</select></div>
            <div style={styles.fieldGroup}><label style={styles.fieldLabel}>Business model</label><select style={styles.selectInput} value={profileDraft.businessModel || ""} onChange={(e) => setProfileDraft({ ...profileDraft, businessModel: e.target.value })}><option value="">Not set yet</option>{BUSINESS_MODELS.map((i) => <option key={i} value={i}>{i}</option>)}</select></div>
      ...
      <Section title="RESTART SETUP">
        <p style={{ fontSize: 12, color: C.muted, marginBottom: 10, lineHeight: 1.5 }}>
          Clears your startup profile and metrics on this device and takes you back through setup. This only affects this device — nothing is deleted from your account on the server.
        </p>
        <button
          style={{ ...styles.ghostBtn, borderColor: "#d85050", color: "#d85050" }}
          onClick={() => {
            if (window.confirm("Restart setup on this device? Your startup profile and metrics stored on this device will be cleared.")) {
