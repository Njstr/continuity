...
import {
  ...
  TrendingUp, Bell, Info, Edit2,
} from "lucide-react";
import { Section, EditableLine } from "../components/common";
import { styles } from "../styles/styles";
import { C, F } from "../styles/theme";
import { FAQS, METRIC_GROUPS, INDUSTRIES, BUSINESS_MODELS, STARTUP_STAGES, COUNTRIES, CURRENCIES } from "../constants";
import { averageError, formatAccuracyPct, accuracyLabel } from "../utils/predictionAccuracy";

// More — the secondary-functionality hub: Company Profile, Startup
// Metrics Management, Decision History, Export Reports, Prediction
// Accuracy, Settings, Help & Support, Feedback, About FounderOS.
export function More({ setScreen, onFeedback, companyProfile, setCompanyProfile, metrics, setMetrics, decisions }) {
  ...
  const [metricsDraft, setMetricsDraft] = useState(metrics);
  ...
  function saveMetrics() {
    setMetrics(metricsDraft);
    setEditingMetrics(false);
  }

  function exportReports() {
    const rows = [["Decision", "Date", "Status", "Overall Confidence", "Feedback Rating", "Avg Simulation Accuracy %"]];
    decisions.forEach((d) => {
      const err = d.accuracyResults?.length ? averageError(d.accuracyResults) : null;
      const acc = err !== null ? Math.max(0, 100 - err) : null;
      rows.push([d.decisionText, d.date, d.status, d.overallConfidence || "", d.feedbackRating || "", acc !== null ? acc.toFixed(1) : ""]);
    });
  ...
  const completedWithAccuracy = decisions.filter((d) => d.status === "completed" && d.accuracyResults?.length);
  const allAccuracy = completedWithAccuracy.flatMap((d) => d.accuracyResults);
  const overallAvgErr = averageError(allAccuracy);
  const overallAcc = accuracyLabel(overallAvgErr);
  ...
  return (
    <div style={styles.screenPad}>
      ...
      <Section title="STARTUP METRICS MANAGEMENT">
        {!editingMetrics ? (
          <button style={{ ...styles.ghostBtn, display: "inline-flex", alignItems: "center", gap: 6 }} onClick={() => { setMetricsDraft(metrics); setEditingMetrics(true); }}>
            <Edit2 size={13} /> Update metrics
          </button>
        ) : (
          <>
            {METRIC_GROUPS.map((g) => (
              <div key={g.key}>
                <div style={styles.metricGroupTitle}>{g.label.toUpperCase()}</div>
                {g.fields.map((f) => (
                  <div key={f.key} style={styles.metricFieldRow}>
                    ...
                    <input
                      ...
                      onChange={(e) => setMetricsDraft({ ...metricsDraft, [f.key]: e.target.value === "" ? null : e.target.value })}
                      placeholder="Unknown"
                    />
                  </div>
                ))}
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
              <button style={styles.primaryBtn} onClick={saveMetrics}><Check size={15} /> Save</button>
              <button style={styles.ghostBtn} onClick={() => setEditingMetrics(false)}>Cancel</button>
            </div>
          </>
        )}
      </Section>
        ...
        {overallAvgErr !== null ? (
          <>
            <p style={styles.missionText}>
              <TrendingUp size={13} style={{ verticalAlign: -2, marginRight: 4 }} color={overallAcc.color} />
              Average Simulation Accuracy across {completedWithAccuracy.length} completed decision{completedWithAccuracy.length === 1 ? "" : "s"}:{" "}
              <b style={{ color: overallAcc.color }}>{overallAcc.emoji} {formatAccuracyPct(overallAvgErr)}</b>
            </p>
            <p style={{ fontSize: 11.5, color: overallAcc.color, marginTop: 4 }}>{overallAcc.label} match between predicted and actual outcomes.</p>
          </>
        ) : (
          <p style={styles.emptyStateText}>No completed decisions with reported outcomes yet.</p>
        )}
        <button style={{ ...styles.ghostBtn, marginTop: 10, display: "inline-flex", alignItems: "center", gap: 6 }} onClick={() => setScreen("learning")}>
          View full Prediction Learning dashboard <ChevronRight size={13} />
        </button>
      </Section>
