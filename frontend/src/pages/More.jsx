...
export function More({ setScreen, onFeedback, companyProfile, setCompanyProfile, metrics, setMetrics, decisions }) {
  ...
  function exportReports() {
    const rows = [["Decision", "Date", "Status", "Overall Confidence", "Feedback Rating", "Avg Error %"]];
    decisions.forEach((d) => {
      const err = d.accuracyResults?.length ? averageError(d.accuracyResults) : null;
      rows.push([d.decisionText, d.date, d.status, d.overallConfidence || "", d.feedbackRating || "", err !== null ? err.toFixed(1) : ""]);
    });
      ...
      <Section title="PREDICTION ACCURACY">
        {overallAvgErr !== null ? (
          <p style={styles.missionText}>
            <TrendingUp size={13} style={{ verticalAlign: -2, marginRight: 4 }} color={C.accent} />
            Average error across {completedWithAccuracy.length} completed decision{completedWithAccuracy.length === 1 ? "" : "s"}: <b style={{ color: C.accent }}>{formatErrorPct(overallAvgErr)}</b>
          </p>
        ) : (
