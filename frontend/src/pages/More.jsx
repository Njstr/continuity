...
export function More({ setScreen, onFeedback, companyProfile, setCompanyProfile }) {
  ...
  return (
    <div style={styles.screenPad}>
      ...
      <Section title="RESET THIS DEVICE">
        <p style={{ fontSize: 12, color: C.muted, marginBottom: 10, lineHeight: 1.5 }}>
          Clears this device's local company profile, metrics, and cached chats, then reloads — useful for re-checking the onboarding flow. This does not touch your account data on the server.
        </p>
        <button
          style={{ ...styles.ghostBtn, borderColor: "#d85050", color: "#d85050" }}
          onClick={() => {
            if (window.confirm("Reset this device and see onboarding again? Your local company profile and metrics on this device will be cleared.")) {
