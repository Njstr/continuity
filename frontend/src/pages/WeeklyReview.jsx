...
import { computeMetrics } from "../utils/businessMetrics";
import { loadJSON, saveJSON } from "../utils/storage";
import { api } from "../api/client";

export function WeeklyReview({ profile, missions, businessState, setScreen }) {
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const businessMetrics = businessState ? computeMetrics(businessState) : null;
      const report = await api.weeklyReview(profile, answers, missions, businessMetrics);
              ...
              <div key={idx} style={styles.faqItem} onClick={() => setExpanded(expanded === idx ? null : idx)}>
                ...
                {expanded === idx && (
                  <div>
                    ...
                    {r.report?.businessSnapshot && (
                      <>
                        <div style={{ ...styles.missionLabel, marginTop: 8, color: C.accent2 }}>TOP IMPROVEMENT</div>
                        <p style={styles.missionText}>{r.report.businessSnapshot.topImprovement}</p>
                        <div style={{ ...styles.missionLabel, marginTop: 8 }}>NEEDS ATTENTION</div>
                        <p style={styles.missionText}>{r.report.businessSnapshot.needsAttention}</p>
                      </>
                    )}
