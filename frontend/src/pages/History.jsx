...
import { computeAccuracy, formatErrorPct } from "../utils/predictionAccuracy";
        ...
        <Section title="PREDICTION ACCURACY">
          {decision.accuracyResults.map((a, idx) => (
            <div key={idx} style={styles.accuracyRow}>
              <span>{a.metric}</span>
              <span style={{ fontFamily: F.mono }}>predicted ~{a.predictedMid?.toFixed?.(0) ?? "—"} · actual {a.actual} · <b style={{ color: C.accent }}>{formatErrorPct(a.errorPct)} error</b></span>
            </div>
          ))}
