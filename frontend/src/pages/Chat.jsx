...
export function Chat({ profile, metrics, conversation, onUpdateMessages, onTitleGenerated, onFeedback, onApplyMetrics }) {
  ...
  // flight, without needing to type anything first. Per the
  // auto-creation behavior, this always resolves to a real task (never
  // null) once the backend has enough to work with.
  useEffect(() => {
    if (!profile) return;
    let cancelled = false;
    api
  ...
  function applyExecutionResult(result) {
    ...
    if (result.event === "completed") {
      // The just-completed task's checkmark state shows for a moment,
      // then fetch what's next through the same auto-creating endpoint
      // used on page load — reliable and idempotent, rather than sending
      // a synthetic chat message to coax the engine into revealing it.
      setTimeout(() => {
        api
          .executionProgress(profile)
          .then(({ current }) => {
            ...
            setActiveTask(current);
            setActiveTaskStep(current.steps?.[current.currentStepIndex]);
            setActiveTaskEvent(current.status === "IN_PROGRESS" ? "task_started" : "in_progress");
            setActiveTaskFeedback(null);
          })
          .catch(() => {});
      }, 1400);
  ...
  async function send() {
    ...
    try {
      if (pendingCheckIn) {
        ...
        // position); the result is rendered via the sticky TaskCard
        // rather than a chat bubble.
        const history = next.slice(-10).map(toHistoryMessage);
        const result = await api.executionMessage(profile, text, history);
        applyExecutionResult(result);
        // final stays `next` — task updates render via the sticky
        // TaskCard, not a separate chat bubble.
      }
    } catch (e) {
