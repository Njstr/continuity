...
export function Chat({ profile, metrics, conversation, onUpdateMessages, onTitleGenerated, onFeedback, onApplyMetrics }) {
  ...
  // Execution engine state (see backend/src/services/executionEngine.js).
  // Deliberately kept separate from `messages` — the task graph is
  // per-founder system-of-record state, not chat history, per the
  // execution-engine spec's "separate chat from task state" requirement.
  // It's rendered as a persistent card, not folded into the message list.
  const [activeTask, setActiveTask] = useState(null);
  const [activeTaskEvent, setActiveTaskEvent] = useState(null);
  const [activeTaskStep, setActiveTaskStep] = useState(null);
  const [activeTaskFeedback, setActiveTaskFeedback] = useState(null);
  ...
  // Task state is per-founder, not per-conversation — load once so a
  // returning founder immediately sees whatever task is already in
  // flight, without needing to type anything first.
  useEffect(() => {
    if (!profile) return;
    let cancelled = false;
    api
      ...
      .then(({ current }) => {
        ...
        setActiveTaskStep(current.steps?.[current.currentStepIndex]);
        setActiveTaskEvent(current.status === "AWAITING_EVIDENCE" ? "insufficient" : "in_progress");
        setActiveTaskFeedback(null);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  // Routes a message straight to the execution engine, bypassing the
  // regex-based intent detectors entirely — used by the TaskCard's
  // evidence box and "I'm stuck" button, where the founder's intent is
  // already unambiguous (they're responding to a specific task/step),
  // not something that should be reinterpreted as a decision-simulator or
  // validation trigger.
  async function submitTaskEvidence(text) {
    setTaskLoading(true);
    try {
      const history = messagesRef.current.slice(-6).map(toHistoryMessage);
      const result = await api.executionMessage(profile, text, history);
      applyExecutionResult(result);
    } catch (e) {
      setActiveTaskFeedback(toFriendlyError(e));
    } finally {
  ...
  function applyExecutionResult(result) {
    if (result.event === "gathering" || result.event === "all_done") {
      // No task exists yet (or everything is done) — this reads like a
      // normal chat reply, not a task update, so it belongs in the
      // message list rather than the sticky card.
      setActiveTask(null);
      const final = [...messagesRef.current, { role: "assistant", content: result.reply, ts: Date.now() }];
      onUpdateMessages(conversationId, final);
      return;
    }
    setActiveTask(result.task);
    setActiveTaskStep(result.currentStep);
    setActiveTaskEvent(result.event);
    setActiveTaskFeedback(result.reply);
    if (result.event === "completed") {
      // Immediately check what's next rather than making the founder
      // type something just to see the next task reveal.
      setTimeout(() => {
        api
          .executionMessage(profile, "(continue)", [])
          .then(applyExecutionResult)
          .catch(() => {});
      }, 1400);
    }
  }
  ...
  async function send() {
    ...
    try {
      if (pendingCheckIn) {
        ...
        // executionEngine.js) — not a generic chat reply. It internally
        // handles founder-state gathering, task generation, step
        // progression, and evidence verification; what comes back here
        // is either a normal reply (still gathering info, or everything
        // currently done) or a task-state update rendered via the
        // sticky TaskCard rather than a chat bubble.
        ...
        if (result.event === "gathering" || result.event === "all_done") {
          ...
          if (result.event === "completed") {
            setTimeout(() => {
              api.executionMessage(profile, "(continue)", []).then(applyExecutionResult).catch(() => {});
            }, 1400);
          }
          // final stays `next` — no separate chat bubble for a task update.
        }
      }
    } catch (e) {
      ...
      <div style={styles.chatScroll}>
        {activeTask && (
          <div style={{ marginBottom: 14 }}>
            <TaskCard
              task={activeTask}
              currentStep={activeTaskStep}
              event={activeTaskEvent}
              feedbackText={activeTaskFeedback}
              onSubmitEvidence={submitTaskEvidence}
            />
            {taskLoading && (
              <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11.5, color: C.muted, marginTop: 6, fontFamily: F.mono }}>
                <Loader2 className="spin" size={12} /> checking...
              </div>
            )}
          </div>
        )}
