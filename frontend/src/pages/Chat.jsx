...
export function Chat({ profile, metrics, conversation, onUpdateMessages, onTitleGenerated, onFeedback, onApplyMetrics }) {
  ...
  async function handleDecisionAct(msgIndex, status, finalDecisionText) {
    ...
    try {
      ...
      onUpdateMessages(conversationId, [...messagesRef.current, { role: "assistant", content: reply, ts: Date.now(), prompt: followUpPrompt }]);
    } catch (e) {
  ...
  async function send() {
    ...
    try {
      if (pendingCheckIn) {
        ...
        const { reply: rawReply } = await api.geoReadinessCheck(profile, history);
        ...
        const { reply: rawReply } = await api.validateIdea(profile, text, history);
        const reply = await finalizeReplyText(rawReply, { profile, metrics, docIds, historyForContinuation: history });
        ...
        // more, or introducing what's next after a completion.
