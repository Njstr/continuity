...
// Plain-text version of a decision simulation card — used as the content
// for Speak/Translate/Feedback on Decision Simulation messages, which
// otherwise store no text (they render as a structured DecisionCard, not
// a text bubble). Closes the "universal action bar" gap: every AI
// response gets like/dislike/speak/translate, decision cards included.
function flattenDecisionSimulation(decisionText, sim) {
  if (!sim) return decisionText || "";
  const lines = [decisionText, ""];
  if (sim.currentSituation) lines.push(sim.currentSituation, "");
  (sim.predictions || []).forEach((p) => lines.push(`${p.metric}: ${p.direction}${p.predictedLow != null ? ` (${p.predictedLow}-${p.predictedHigh})` : ""} — ${p.reasoning || ""}`));
  if (sim.bestCase) lines.push("", `Best case: ${sim.bestCase}`);
  if (sim.expectedCase) lines.push(`Expected: ${sim.expectedCase}`);
  if (sim.worstCase) lines.push(`Worst case: ${sim.worstCase}`);
  if (sim.keyAssumptions?.length) lines.push("", `Assumptions: ${sim.keyAssumptions.join("; ")}`);
  if (sim.mainRisks?.length) lines.push(`Risks: ${sim.mainRisks.join("; ")}`);
  return lines.filter(Boolean).join("\n");
}

// Every place that builds an AI-request history from the message array
// must go through this — decision-simulation messages store content: ""
// (they render as a structured DecisionCard, not a text bubble), and an
// empty-content assistant message sent straight to the AI provider is
// what was breaking conversations permanently after Proceed/Modify/Not
// now (see handoff doc §9/§13). Do not map { role: m.role, content: m.content }
// directly anywhere — always route through this helper instead.
function toHistoryMessage(m) {
  return {
    role: m.role,
    content: m.decisionSimulation ? flattenDecisionSimulation(m.decisionText, m.decisionSimulation) : m.content,
  };
}

// ---- Response Completion & Continuity (frontend safeguard) ----
// The backend (aiService.completeAIReply) already guarantees every chat
// reply it returns is a complete thought — this is a second, independent
// layer for what the backend can't see: a response that was complete
// when the server sent it but arrived truncated here (a flaky network
// connection, a proxy cutting the body short, an older cached bundle
// talking to a backend that predates this fix). Mirrors the backend
// heuristic in aiService.js — kept in sync deliberately, not shared code,
// since frontend/backend are separate runtimes here.
const DANGLING_ENDINGS_RE = /^(because|so|but|and|or|if|when|which|that|while|as|though|although|since|for|nor|yet|to|with|of|in|on|at|the|a|an)$/i;
...
function looksIncomplete(text) {
  ...
  if (DANGLING_ENDINGS_RE.test(trailingWord) && !/[.!?]\s*$/.test(t)) return true;
  const isBulletLine = /^([-*•]|\d+[.)])\s+\S/.test(lastLine);
  if (!isBulletLine && !/[.!?)\]"'’”]\s*$/.test(t)) return true;
  return false;
}

// Called around every prose reply the founder is about to see. Attempts
// one continuation through the normal chat pipeline if the reply looks
// cut off; if that doesn't produce a complete thought either, returns a
// graceful, honest fallback instead of ever silently saving a visibly
// truncated message as the final assistant reply.
async function finalizeReplyText(rawReply, { profile, metrics, docIds, historyForContinuation }) {
  if (!looksIncomplete(rawReply)) return rawReply;
  ...
  try {
    ...
    const { reply: continuation } = await api.chat(profile, [], continuationHistory, undefined, undefined, metrics, docIds);
    const merged = `${rawReply}${rawReply.endsWith(" ") || continuation.startsWith(" ") ? "" : " "}${continuation}`.trim();
    if (!looksIncomplete(merged)) return merged;
    // Still not complete after one real attempt — don't keep trying
    // indefinitely against the user's connection; fall through to the
    // graceful fallback below.
  } catch {
    // Continuation attempt itself failed (network error, etc.) — fall
    // through to the graceful fallback rather than surfacing that error
    // on top of an already-truncated reply.
  }

  return "I started an answer but couldn't finish it cleanly that time — want me to try that again?";
}
...
export function Chat({ profile, metrics, conversation, onUpdateMessages, onTitleGenerated, onFeedback, onApplyMetrics }) {
  ...
  // Tasks from the execution engine (see backend/src/services/
  // executionEngine.js) are no longer a separate card/dashboard — they're
  // just conversation messages, like everything else FounderOS says. This
  // effect's only job is the one-time reveal: if the founder's current
  // task has never been announced anywhere yet (status still AVAILABLE),
  // inject the natural "Today's task: ..." message once and let the
  // backend mark it announced (AVAILABLE -> IN_PROGRESS) so it's never
  // repeated — including across page refreshes and other conversation
  // threads, since the announced-state lives on the task itself, not in
  // this component or this one conversation's history.
  //
  // revealCheckedRef guards against firing this GET twice for the same
  // conversation — it's not a pure read: the backend transitions the
  // task's status as a side effect of checking, so a second call after
  // the first already landed correctly (but unhelpfully) reports nothing
  // left to announce. A plain per-invocation `cancelled` closure variable
  // isn't enough to prevent that: React StrictMode deliberately runs this
  // effect's mount → cleanup → mount cycle once in dev, and each
  // invocation gets its own fresh `cancelled`, so the first (successful)
  // call ends up self-cancelled while the second, now-uninformative call
  // is the one left to apply. A ref persists across that whole cycle
  // (same instance both times), so it can stop the second call from ever
  // being fired at all, rather than racing to suppress its result after
  // the fact.
  const revealCheckedRef = useRef(new Set());
  useEffect(() => {
    if (!profile || !conversationId || revealCheckedRef.current.has(conversationId)) return;
    revealCheckedRef.current.add(conversationId);
    // No `cancelled`/cleanup-based guard here on purpose: revealCheckedRef
    // above already guarantees at most one network call ever fires for a
    // given conversationId (including across React StrictMode's dev-only
    // mount → cleanup → mount cycle), so there is never a second,
    // superseding request that would make discarding this one's result
    // correct — it's the only request there'll ever be for this
    // conversation, and its result should always be applied.
    api
      .executionProgress(profile)
      .then(({ needsAnnouncement, announcementText }) => {
        if (!needsAnnouncement || !announcementText) return;
        onUpdateMessages(conversationId, [...messagesRef.current, { role: "assistant", content: announcementText, ts: Date.now() }]);
      })
      .catch((e) => {
        // A failed task-progress check shouldn't block the founder from
        // using chat normally, but it must be visible somewhere real
        // rather than vanishing — silently swallowing errors on the task
        // system is exactly the pattern the production-readiness pass
        // fixed for the rest of the app, and it applies here too.
        // eslint-disable-next-line no-console
        console.error("[Chat] execution progress check failed:", e);
        revealCheckedRef.current.delete(conversationId); // allow a retry on the next relevant render rather than blackholing this conversation forever after one network blip
      });
  ...
  async function maybeGenerateTitle(finalMessages) {
    ...
    const realMessages = finalMessages.filter((m) => m.role === "user" || m.role === "assistant").map(toHistoryMessage);
  ...
  async function handleFiles(fileList) {
    ...
    try {
      ...
      setAttachments((prev) => prev.map((a) => (pendingIds.includes(a.id) ? { ...a, status: "error", error: toFriendlyError(e) } : a)));
    }
  }
  ...
  async function handleDecisionAct(msgIndex, status, finalDecisionText) {
    ...
    if (!msg?.decisionText || !profile) return;

    // Step 1: record the decision lifecycle FIRST, before generating
    // anything — this is durable the moment the button click succeeds,
    // independent of whether the follow-up AI call below works.
    try {
      ...
      // Recording itself failed — nothing was saved, card stays fully
      // actionable so the founder can just click again.
      onUpdateMessages(conversationId, [
        ...messagesRef.current,
        { role: "assistant", content: toFriendlyError(e), ts: Date.now() },
      ]);
      return;
    }
    ...
    // Step 2: continue the SAME conversation with an intelligent
    // follow-up — not a bare acknowledgment. Reuses the existing chat()
    // pipeline (memory, reasoning, decision history, currency
    // formatting) instead of a separate, duplicated code path, via a
    // synthetic user turn describing what was just decided.
    const followUpPrompt =
      status === "proceeded"
        ? `I've decided to proceed with this: "${finalDecisionText}". Walk me through why this is reasonable given everything you know, the assumptions that need to hold, the biggest execution risks, milestones and early warning signs to watch for, and my first concrete actions.`
        : status === "modified"
        ? `I'm modifying the original decision ("${msg.decisionText}") to: "${finalDecisionText}". How does this change affect the prediction — does risk go up or down, and what are the trade-offs versus the original?`
        : `I've decided not to act on this right now: "${msg.decisionText}". What information am I actually missing, what should I go find out, whether waiting is strategically sound here, and what signal should make me revisit this.`;

    setSending(true);
    try {
      const history = [
        ...messagesRef.current.slice(-8).map(toHistoryMessage),
        { role: "user", content: followUpPrompt },
      ];
      const { reply, sources } = await api.chat(profile, [], history, undefined, undefined, metrics, []);
      onUpdateMessages(conversationId, [...messagesRef.current, { role: "assistant", content: reply, ts: Date.now(), prompt: followUpPrompt, sources }]);
    } catch (e) {
      // The decision is already safely recorded (step 1 succeeded) —
      // only the follow-up commentary failed. Say so plainly and let
      // them just ask again in chat; never silently drop it, never
  ...
  async function send() {
    ...
    try {
      if (pendingCheckIn) {
        ...
        const reply = await finalizeReplyText(parts.join("\n"), { profile, metrics, docIds, historyForContinuation: next.slice(-10).map(toHistoryMessage) });
        ...
        const history = next.slice(-4).map(toHistoryMessage);
        const { reply: rawReply, sources } = await api.geoReadinessCheck(profile, history);
        const reply = await finalizeReplyText(rawReply, { profile, metrics, docIds, historyForContinuation: history });
        final = [...next, { role: "assistant", content: reply, ts: Date.now(), geoChecked: true, prompt: text, sources }];
        ...
        const history = next.slice(-6).map(toHistoryMessage);
        const { reply: rawReply, sources } = await api.validateIdea(profile, text, history);
        const reply = await finalizeReplyText(rawReply, { profile, metrics, docIds, historyForContinuation: history });
        final = [...next, { role: "assistant", content: reply, ts: Date.now(), validated: true, prompt: text, sources }];
        ...
        // Default behavior: the evidence-gated execution engine (see
        // executionEngine.js) — not a generic chat reply. It always
        // resolves to a real task (auto-creating one if none exists yet,
        // including a real "tell me what you're building" task when
        // there isn't enough founder info — never an invented startup
        // position). Tasks aren't a separate card/dashboard — the reply
        // is just the next natural message in the conversation, whether
        // that's revealing a task, acknowledging evidence, asking for
        ...
        final = [...next, { role: "assistant", content: reply, ts: Date.now(), prompt: text, sources: result.sources }];
      }
    } catch (e) {
      final = [...next, { role: "assistant", content: toFriendlyError(e), ts: Date.now() }];
    }
  ...
  return (
    ...
    >
      ...
      <div style={styles.chatScroll}>
        <div style={styles.chatContentCol} className="chat-content-col">
          {messages.length === 0 ? (
            ...
            <div key={idx} style={{ width: "100%" }}>
              <div style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: m.role === "user" ? "flex-end" : "flex-start", gap: 4, maxWidth: m.decisionSimulation ? "100%" : undefined }}>
                  ...
                  {m.role === "assistant" && m.sources?.length > 0 && <SourcesList sources={m.sources} />}
              ...
              {m.role === "assistant" && (
                <FeedbackWidget
                  context={
                    m.decisionSimulation ? "decision_simulation" : m.geoChecked ? "geo_readiness" : m.validated ? "product_validation" : m.compared ? "growth_comparison" : "chat"
                  }
                  content={m.decisionSimulation ? flattenDecisionSimulation(m.decisionText, m.decisionSimulation) : m.content}
                  ...
                  meta={{
                    prompt: m.prompt,
                    simulated: !!m.decisionSimulation,
                    simulationResult: m.decisionSimulation || m.comparisonResult,
      ...
      <div style={styles.chatInputBar} className="chat-composer-bar">
        <div style={styles.chatInputRow} className="chat-content-col">
          <input
            ref={fileInputRef}
            type="file"
