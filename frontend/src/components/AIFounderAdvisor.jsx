...
import { Loader2, RefreshCw, ChevronDown, ChevronUp, Zap, AlertTriangle, TrendingUp, Lightbulb, Search, CalendarClock } from "lucide-react";
...
// AIFounderAdvisor — synthesizes companyProfile, metrics, decision history,
// and the cached startup health score into one mentor-style briefing.
// Generation logic lives entirely in api.founderAdvisor / aiService's
// generateFounderAdvisor on the backend — this component only renders
// whatever JSON comes back, so swapping the LLM provider never touches
// this file. Lives inside the Simulator tab as its own self-contained
// section, independent of whether a simulation has been run.
export function AIFounderAdvisor({ companyProfile, metrics, decisions }) {
  ...
  async function run() {
    ...
    try {
      ...
      const result = await api.founderAdvisor(companyProfile, metrics, decisions, healthLatest);
      setData(result);
