...
export function Header({ companyProfile, onToggleSidebar }) {
  return (
    <div style={styles.header}>
      ...
      {companyProfile.stage && <div style={styles.stageBadge}>{companyProfile.stage}</div>}
    </div>
  );
}

// Exactly three primary tabs, always — non-negotiable per the product
// spec. Everything else (decisions, company profile, documents, GEO,
// validation, comparisons) lives inside Chats or More, never as its own
// tab or route.
const NAV_ITEMS = [
...
const IMPLICIT_MORE_SCREENS = ["terms", "learning"];
