...
/* Chat composer — full viewport width on phones/tablets (existing
   behavior, unchanged), centered and width-capped on laptop/desktop like
   ChatGPT/Claude. The outer bar (.chat-composer-bar) stays full-width so
   its border and background still span the whole panel; only the inner
   row (.chat-composer-row — the actual attach/input/send controls) gets
   constrained and centered above the breakpoint. */
@media (min-width: 1024px) {
  .chat-composer-row { max-width: 960px; margin: 0 auto; }
}
`;
