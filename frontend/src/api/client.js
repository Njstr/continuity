...
export const api = {
  // ---- AI generation ----
  onboard: (description) => request("/api/onboarding", { body: { description } }),
  ...
  // ---- Decision lifecycle (Decide / Decisions / Today) ----
  ...
  // ---- Decision lifecycle (steps 4-9: simulate -> record -> predict -> observe -> compare -> learn) ----
  simulateDecisionForRecord: (profile, metrics, decisionText, decisionContext) =>
    request("/api/decisions/simulate", { body: { profile, metrics, decisionText, decisionContext } }),
  recordDecision: (decisionText, finalDecisionText, rationale, status, simulation, companySnapshot) =>
    request("/api/decisions", { body: { decisionText, finalDecisionText, rationale, status, simulation, companySnapshot } }),
  getDueDecisions: () => request("/api/decisions/due", { method: "GET" }),
  recordDecisionOutcome: (decisionId, actualUpdate, actualMetrics) =>
    request(`/api/decisions/${decisionId}/outcome`, { body: { actualUpdate, actualMetrics } }),

  extractChatMetrics: (text, metricFields) => request("/api/extract-chat-metrics", { body: { text, metricFields } }),
