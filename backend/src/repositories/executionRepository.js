...
const TASK_COLUMNS = `
  ...
  action_type AS actionType, autonomous_work AS autonomousWork, execution_state AS executionState,
  created_at AS createdAt, updated_at AS updatedAt, completed_at AS completedAt
`;

const insertTaskStmt = db.prepare(`
  INSERT INTO tasks (id, user_id, title, objective, why_it_matters, dependencies, steps, current_step_index, completion_criteria, evidence_requirements, required_threshold, priority_factors, priority_score, status, evidence_config, category, objective_key, revisit_conditions, reason, why_now, expected_outcome, context_references, action_type, autonomous_work, execution_state, created_at, updated_at)
  VALUES (@id, @userId, @title, @objective, @whyItMatters, @dependencies, @steps, 0, @completionCriteria, @evidenceRequirements, @requiredThreshold, @priorityFactors, @priorityScore, @status, @evidenceConfig, @category, @objectiveKey, @revisitConditions, @reason, @whyNow, @expectedOutcome, @contextReferences, @actionType, @autonomousWork, @executionState, @createdAt, @updatedAt)
`);
...
function parseTaskRow(row) {
  ...
  return {
    ...
    // Pre-011 rows have actionType = NULL — executionEngine treats that
    // as FOUNDER_REQUIRED (every task under the old model WAS a
    // founder-facing task), never as an error.
    autonomousWork: fromJSON(row.autonomousWork, []),
  };
}

function createTask({ userId, title, objective, whyItMatters, dependencies, steps, completionCriteria, evidenceRequirements, requiredThreshold, priorityFactors, priorityScore, status, evidenceConfig, category, objectiveKey, revisitConditions, reason, whyNow, expectedOutcome, contextReferences, actionType, autonomousWork, executionState }) {
  const id = uid("task");
  const ts = nowISO();
  insertTaskStmt.run({
    id,
    ...
    autonomousWork: toJSON(autonomousWork || []),
...
// §14 of the autonomous-execution spec — the richer execution-state label
// layered on top of `status` (see migration 011's comment for why this
// isn't a duplicate state system). Used for transitions `status` alone
// can't express, e.g. a founder-required task moving from PLANNING (while
// FounderOS is doing autonomous prep for it) to WAITING_FOR_FOUNDER once
// that prep is done and it's actually shown.
const updateExecutionStateStmt = db.prepare(`UPDATE tasks SET execution_state = ?, updated_at = ? WHERE id = ? AND user_id = ?`);
function setExecutionState(id, userId, executionState) {
  updateExecutionStateStmt.run(executionState, nowISO(), id, String(userId));
  return getTask(id, userId);
}
...
module.exports = {
  ...
  setStatus, setExecutionState, setCurrentStep, recordEvidence, recordOutcome, setPriority, addDependencyAndLock,
