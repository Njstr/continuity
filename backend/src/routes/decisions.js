  ...
  asyncHandler(async (req, res) => {
    ...
    activityEvidenceEngine
      .recordActivity(req.userId, {
        ...
        metadata: { status, decisionText },
        description: `Recorded a decision: "${finalDecisionText || decisionText}" (${status})`,
        dedupeKey: `decision_recorded:${decision.id}`,
      })
      .catch((e) => console.error(`[decisions] activity recording failed for decision ${decision.id}:`, e.message));
  ...
  asyncHandler(async (req, res) => {
    ...
    activityEvidenceEngine
      .recordActivity(req.userId, {
        ...
        description: `Recorded the outcome of a past decision: ${actualUpdate}`,
        dedupeKey: `decision_outcome_recorded:${outcome.id}`,
      })
      .catch((e) => console.error(`[decisions] activity recording failed for outcome ${outcome.id}:`, e.message));
