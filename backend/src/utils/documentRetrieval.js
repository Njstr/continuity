...
function chunkText(text, filename) {
  ...
  while (start < text.length) {
    ...
    if (end < text.length) {
      // Prefer breaking at a blank line (paragraph or table boundary)
      // over a hard character cut — tables are dense with meaning per
      // row, and splitting one mid-row across two chunks is exactly the
      // kind of thing that silently produces a wrong answer later.
      const lastBlankLine = text.lastIndexOf("\n\n", end);
      if (lastBlankLine > start + CHUNK_SIZE * 0.4) end = lastBlankLine;
    }
