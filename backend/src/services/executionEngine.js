...
async function _resolveCurrentTask(userId, state) {
  ...
  return selectCurrentTask(userId) || createFallbackTask(userId, state || repo.getFounderState(userId) || {});
}
...
async function ensureCurrentTask(userId, profile) {
  return withFounderLock(userId, async () => {
    ...
    if (preexisting) return { task: preexisting, bootstrapped: false };
    const task = await _resolveCurrentTask(userId);
    return { task, bootstrapped: true };
  });
}
...
async function handleOnboardingMessage(userId, { profile, text, recentHistory }) {
  return withFounderLock(userId, async () => {
    ...
    if (priorState?.ready) {
      const task = await _resolveCurrentTask(userId, priorState);
      return { event: "task_started", reply: describeTaskReveal(task), task, currentStep: task.steps?.[task.currentStepIndex] };
    }
    ...
    const task = await _resolveCurrentTask(userId, saved);
    if (task.status === "AVAILABLE") {
      const started = repo.setStatus(task.id, userId, "IN_PROGRESS");
      return { event: "task_started", reply: `${reply}\n\n${describeTaskReveal(started)}`, task: started, currentStep: started.steps[started.currentStepIndex] };
    }
    return { event: "task_started", reply, task, currentStep: task.steps?.[task.currentStepIndex] };
  });
}
...
module.exports = { handleMessage, selectCurrentTask, ensureCurrentTask, getProgressSummary, priorityScore, verifyEvidence, buildStartupContext, determineNextAction, handleOnboardingMessage };  founderLocks.set(key, run.catch(() => {}));
  return run;
}
