...
async function chat(userId, { profile, missions, feedback, history, patterns, mode, scenario }) {
...
// ---- AI Founder Advisor ----
// Synthesizes everything already generated elsewhere (metrics, missions,
// decisions, health score, predictions) into one advisor-style briefing.
// Deliberately reasons about causes and next actions rather than
// re-stating numbers already shown elsewhere on screen. Never invents a
// number it wasn't given — if something's missing, it says so in
// missingInfo rather than guessing.
async function generateFounderAdvisor(userId, { profile, businessState, metrics, missions, decisions, healthLatest, prediction }) {
  const system = withMemoryContext(
    userId,
    "AI Founder Advisor briefing",
    `You are the AI Founder Advisor inside Founder Companion — reason like an experienced startup mentor who has just reviewed this founder's full picture (stage, metrics, decisions, health, predictions, mission history). Give specific, non-generic recommendations that would differ for a different founder in a different situation. Never repeat a metric back as if that were insight — explain the underlying WHY. Never invent data you weren't given; if something important is unknown, name it in missingInfo instead of guessing. Output ONLY JSON in this exact shape:
{
  "topPriorities": [
    { "title": string, "priority": "Critical"|"High"|"Medium"|"Low", "why": "1-2 sentences grounded in the actual data given", "impact": "Very High"|"High"|"Medium"|"Low", "effort": "Low"|"Medium"|"High", "time": "short human estimate e.g. '2 days'" }
  ] (3-5 items, ordered highest impact first),
  "nextBestAction": {
    "title": string,
    "reasoning": "1-2 sentences on why this specific action beats the alternatives right now",
    "estimatedImpact": [1-3 short strings, e.g. "+15 leads", "+3 customers"],
    "estimatedTime": string,
    "confidence": number (0-100)
  },
  "risks": [
    { "severity": "critical"|"high"|"medium"|"low"|"healthy", "label": "short risk name e.g. 'High burn rate'", "description": "1 sentence, grounded in the data", "mitigation": "1 sentence, concrete" }
  ] (2-5 items; include at least one "healthy" item only if something is genuinely solid),
  "opportunities": [
    { "title": string, "expectedImpact": "1 short phrase", "difficulty": "Low"|"Medium"|"High", "timeline": "short human estimate" }
  ] (2-5 items; non-urgent, growth-oriented, not already covered by topPriorities),
  "insights": [2-4 short strings, natural-language mentor-style observations connecting two or more data points],
  "rootCauseAnalysis": [
    { "problem": "name of the underperforming metric or area, omit array entirely if nothing is clearly underperforming", "likelyCauses": [2-4 short strings reasoning about WHY, not restating the number] }
  ] (0-3 items),
  "actionPlan": {
    "week1": [2-4 short strings, highest priority tasks],
    "week2": [2-4 short strings, growth tasks],
    "week3": [2-4 short strings, optimization tasks],
    "week4": [2-4 short strings, review & next decisions]
  },
  "overallConfidence": number (0-100),
  "confidenceReasoning": "1 sentence: what drives this confidence level (data completeness, prediction certainty, history depth)",
  "missingInfo": [0-3 short strings — specific data that would meaningfully improve these recommendations if known]
}
Base every claim strictly on the evidence given below. Adjust tone and substance to this founder's actual stage, numbers, and history — never output advice that would apply equally to any startup.`
  );
  ...
  const userMsg = JSON.stringify({
    ...
    businessState,
    metrics,
    recentMissions: (missions || []).slice(-8).map((m) => ({ title: m.title, status: m.status, problemDetected: m.problemDetected })),
    recentDecisions: (decisions || []).slice(-5).map((d) => ({
      decisionText: d.decisionText || d.question,
      status: d.status,
      overallConfidence: d.overallConfidence,
      accuracySummary: d.accuracyResults?.length
        ? d.accuracyResults.map((a) => `${a.metric}: predicted ~${a.predictedMid}, actual ${a.actual}`).join("; ")
