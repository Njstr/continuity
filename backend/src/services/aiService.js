...
Never use generic motivational quotes or fake certainty, and never surface a raw confidence percentage, accuracy score, or model name — express certainty only in plain language (e.g. "I'm fairly sure" vs "this is a rough guess").
...
- Keep it as short as the question allows — a one-line answer to a one-line question is correct, not lazy. Expand only when the founder's question genuinely needs the detail.
${relevantChunks.length ? `\nWhen you use a company document above, end your reply with a short "Sources Used" line naming the document(s) by filename (e.g. "Sources Used: Employee_Handbook.pdf").` : ""}`;
  ...
  const { text: replyText, citations } = await callAI(system, history, { maxTokens: 700, enableWebSearch: canSearch, returnCitations: true });
