 ...
 */
async function callAI(system, messages, { json = false, shape, maxTokens = 1000, enableWebSearch = false, enableTools = false, returnCitations = false, returnMeta = false } = {}) {
      ...
      withTimeout(
        (signal) =>
          provider.complete(system, messages, {
            maxTokens,
            signal,
            enableWebSearch: enableWebSearch && provider.supportsWebSearch,
            ...(enableTools && provider.supportsTools ? { tools: aiTools.TOOLS, toolExecutor: aiTools.executeTool, maxToolRounds: 3 } : {}),
          }),
...
async function chat(userId, { profile, missions, feedback, history, patterns, recentDecisions, learnedPatterns, mode, scenario, metrics, documents }) {
  ...
  if (mode === "simulator" && scenario) {
    ...
    const reply = await completeAIReply(system, history, { maxTokens: 1000 });
  ...
  // check the live web instead of leaning on static training data.
  const isLegalOrRegulatoryQuestion = LEGAL_REGULATORY_PATTERN.test(lastUserMsg);
  // Whether the model has any real search capability this turn at all —
  // used only to shape what the jurisdiction guidance below says is
  // possible. The actual web_search tool (SearXNG-backed — see
  // services/aiTools.js) is offered unconditionally below regardless of
  // topic; whether to invoke it for THIS message is the model's own
  // judgment, not a topic-based gate (see the tool's own description for
  // when it should be used) — the ONLY thing gated on
  // isLegalOrRegulatoryQuestion is the wording of the jurisdiction
  // instruction the model reads, not whether search is available.
  const canSearch = provider.supportsTools;
  ...
  const jurisdictionBlock = `\nFOUNDER'S JURISDICTION: ${profile.country || "not specified"}. When advice touches law, tax, employment, compliance, or regulation, ground it specifically in this jurisdiction — not generic "startup wisdom." Laws frequently vary by state/province/region WITHIN a country too; say so explicitly rather than implying one national answer covers every region. ${
    canSearch
      ? isLegalOrRegulatoryQuestion
        ? "This looks like a law/tax/compliance question — use the web_search tool to check current law/rates/regulations rather than relying on your training data, which may be outdated."
        : "You have the web_search tool available if a question ever needs current external verification."
...
${canSearch ? `\nWEB SEARCH: you have a web_search tool backed by a real search engine. Use it when the founder is asking you to find/research/compare something, wants current information ("what's the latest...", "what are people saying about..."), wants to find communities or discussions (e.g. Reddit — you can search site:reddit.com or site:reddit.com/r/ to find relevant subreddits, and you can tell a community page apart from an individual post by the URL shape), or anything else time-sensitive or external that the conversation itself can't answer reliably. Don't search for things you already know that don't change, or things fully answerable from context already in this conversation. For a genuinely multi-part research question you can search more than once (e.g. a general search plus a Reddit-scoped one) — stay purposeful, not exhaustive. Never state a fact as coming from a search unless a search actually returned it; if you're answering from your own knowledge instead, don't imply otherwise.` : ""}`;

  const { text: reply, citations } = await completeAIReply(system, history, { maxTokens: 450, enableTools: canSearch, returnCitations: true });
  ...
  // Sources are returned as structured data (§7/§8), not flattened into
  // the reply text — the frontend renders them as real clickable source
  // cards distinct from the AI's prose, and can never end up displaying
  // an invented source: this array only ever contains URLs a real
  // web_search call actually returned (see services/aiTools.js).
...
Never invent a "connect your startup to ChatGPT" step — it doesn't exist. Everything here should be something the founder can literally publish or do today. Always finish on a complete statement or, if genuinely needed, a complete question — never end mid-sentence, mid-bullet, or on a trailing colon/conjunction.`;
  ...
  const { text: reply, citations } = await completeAIReply(system, [...recentHistory, { role: "user", content: "Run a GEO / AI-visibility check for my startup." }], {
    maxTokens: 500,
...
A founder who hears "unclear, go talk to 10 people directly" and acts on it is better off than one who hears an inflated "yes, people want this." Never invent a thread, quote, subreddit, or number that wasn't actually found. Always finish on a complete statement or, if genuinely needed, a complete question — never end mid-sentence, mid-bullet, or on a trailing colon/conjunction.`;
  ...
  const recentHistory = (history || [])
    ...
    .map((m) => ({ role: m.role, content: m.content }))
    .filter((m) => typeof m.content === "string" && m.content.trim() !== "");
  const { text: reply, citations } = await completeAIReply(system, [...recentHistory, { role: "user", content: idea }], {
    maxTokens: 500,
    enableWebSearch: canSearch,
    returnCitations: true,
  });
...
//
// This is the ONE call that runs for every turn of the pre-task
// onboarding conversation (fixing the "feels like a form" bug): it
// extracts+merges whatever the founder just said into the founder_state
// shape, decides readiness, AND drafts the exact natural-language reply
// shown to the founder — all in one pass, so the reply is guaranteed to
// be grounded in (and never contradict) what was just extracted, and so
// there is no separate "internal reasoning" string that some other layer
// might accidentally surface verbatim. Nothing about FounderOS's internal
// field list, validation logic, or "critical vs secondary" tiering is
// ever supposed to reach the reply text — see the explicit rules
...
// don't hold the founder hostage for them).
...
async function synthesizeOnboardingTurn(userId, { profile, priorState, recentMessages }) {
  const system = withMemoryContext(
    userId,
    "founder onboarding conversation",
    `You are FounderOS, an intelligent founder companion having a natural first conversation with a founder to understand who they are and what they're building. This is a CONVERSATION, not a form — the founder must never feel like they're filling out fields, and must never see or sense that a checklist of required information exists.

Founderos privately tracks these fields over time:
- Founder: founderName, founderRole, founderExperience, founderSkills, founderGoals
- Startup: startupName, goal, currentMilestone, stage, problem, targetCustomer, currentSolution, businessModel, traction, revenue, competitors, fundingStatus
- Working picture: validationLevel, evidenceCollected, resources, constraints, majorAssumptions, knownRisks, currentBottleneck

Of these, the CRITICAL fields that must be known before FounderOS can respond with a real, specific next action are: ${CRITICAL_READINESS_FIELDS.join(", ")}. The rest (${SECONDARY_READINESS_FIELDS.join(", ")}, and the working-picture fields) are useful context but must never block or be demanded.

priorState (if given) is everything already known — merge, don't discard: keep every prior value the founder's new message doesn't update or contradict. If the founder's latest message states MULTIPLE pieces of information at once, extract and store ALL of them, not just the one you expected — never lose a piece of information because you were only listening for one answer.

Then, based on the FULLY MERGED picture:
1. If any CRITICAL field is still missing: write ONE natural reply that (a) genuinely and specifically acknowledges what the founder just told you — reflect back the actual substance in your own words, briefly, never a generic "got it" alone — then (b) asks ONE natural, conversational question about the single most important still-missing CRITICAL field. Ask about exactly one thing. Never ask about anything already known from priorState or from this message, even implicitly.
2. If every CRITICAL field is now known (including from this message): write a short, warm reply that acknowledges the final piece of information — do NOT ask another question, and do NOT recite or summarize every field back like a form confirmation. A task will be shown to the founder immediately after your reply, in the same message, so keep this brief and let it transition naturally (e.g. "Got it — that's exactly what I needed to get started.").

Absolute rules for the reply text — violating any of these is a failure:
- NEVER say or imply anything like "FounderOS doesn't know enough," "I need more information," "critical information," "required field," or reference validation/task-generation logic in any way. The founder should never be able to tell a checklist exists.
- NEVER phrase this as a form prompt ("Please provide your target customer:"). Ask the way a sharp, curious cofounder would ask in conversation, referencing what they just said.
- NEVER re-ask for something already known from priorState or the current message.
- Keep it concise — 1-3 sentences, not a paragraph.
- Do not generate or mention a task in this reply text — that happens separately, only once truly ready.

Output ONLY JSON, with every founder_state field below PLUS ready and reply:
{
  ...
  "goal": "what the startup is ultimately trying to achieve right now, in their own terms, or null",
  "currentMilestone": "the nearer-term checkpoint on the way to that goal, or null if not yet clear",
  "stage": "idea"|"validating"|"building"|"launched"|"early_traction"|"revenue"|"growth"|null,
  "problem": "the problem being solved, as understood so far, or null if unknown",
  "targetCustomer": "who this is for, as specifically as currently known, or null if unknown",
  "currentSolution": "what exists today, if anything — 'nothing built yet' is a valid, honest value, or null if not yet discussed",
  "businessModel": "how the startup makes or intends to make money, or null if not yet discussed",
  "traction": "concrete signs of traction so far, or null if none yet/not discussed",
  "revenue": "revenue status as described (can be 'none yet'), or null if not discussed",
  "competitors": ["short list of named competitors/alternatives mentioned — empty array if none"],
  "fundingStatus": "funding situation as described, or null if not discussed",
  "validationLevel": "none"|"anecdotal"|"some_evidence"|"strong_evidence",
  "evidenceCollected": ["short list of concrete evidence that actually exists — empty array if none"],
  "resources": "time/money/skills available, as far as known, or null",
  "constraints": ["short list of real constraints mentioned — empty array if none"],
  "majorAssumptions": ["1-4 things being assumed true that haven't been tested — empty array if none yet clear"],
  "knownRisks": ["1-4 short risk statements — empty array if none yet clear"],
  "currentBottleneck": "one sentence on the biggest thing blocking progress, grounded in what's known, or null if not yet clear",
  "ready": true|false — true once every CRITICAL field (${CRITICAL_READINESS_FIELDS.join(", ")}) has a genuine, non-vague value,
  "reply": "the exact natural-language message to show the founder, following the rules above"
}
Use null (not empty strings, not guesses) for anything genuinely not yet known. Never invent specifics that weren't actually said.`
  );
  const userMsg = JSON.stringify({ profile, priorState: priorState || undefined, recentMessages: (recentMessages || []).slice(-16) });
  return callAI(system, [{ role: "user", content: userMsg }], {
    json: true,
    shape: { requiredKeys: ["ready", "reply"] },
    maxTokens: 900,
  });
}
...
Output ONLY JSON:
{
  ...
  "currentMilestone": "the nearer-term checkpoint on the way to that goal — the concrete thing that needs to be true soon, e.g. '10 completed customer interviews', 'first paying customer', 'onboarding completion above 50%' — or null if not yet clear enough to name one",
...
// what's already completed (§4, §12, §22 of the execution-engine spec;
// §1-§11 of the context-aware-task-generation spec). Deliberately
// generates a handful at a time rather than a whole roadmap up front —
// the founder only ever sees one anyway, and generating fresh after each
// completion is what makes real reprioritization actually happen instead
// of just working down a list decided on day one.
...
// §4 of the autonomous-execution spec — every candidate action must be
// classified before it's ever shown to the founder as a "task." Kept to
// exactly the three values the spec defines, and deliberately narrow on
// what counts as AUTONOMOUS: FounderOS's only real executable capability
// right now is live web research (search + fetch + read — see
// webResearchService.js) plus reasoning/analysis over what's already
// known. Nothing else in this codebase can send an email, publish
// anything, or take any other external action — so ACTION_TYPES exists
// as a real, reasoned-about classification, but AUTONOMOUS in practice
// only ever means "this is answerable via research/analysis," and
// APPROVAL_REQUIRED currently has no real executor behind it at all (see
// executionEngine.js's own comment on this — no fake tool is created to
// pretend otherwise).
const ACTION_TYPES = ["AUTONOMOUS", "APPROVAL_REQUIRED", "FOUNDER_REQUIRED"];

// This is the entire "context object" the context-aware-task-generation
// spec's §14 asks for — assembled by executionEngine.buildStartupContext,
// not invented here. This function only ever REASONS over context handed
// to it; it never fetches anything itself. See buildStartupContext's own
// comment for exactly what's collected and why.
async function proposeNextTasks(
  userId,
  { founderState, completedTasks, recentlyCompletedWithOutcomes, recentChanges, blockedTasks, recentDecisions, activeTaskTitles, excludedObjectiveKeys }
) {
  const system = withMemoryContext(
    userId,
    "execution task generation",
    `You are the action planner for FounderOS — an execution system, not a task-generation system. Your job is NOT "generate a startup task" — it is: given everything currently known about THIS founder and THIS startup RIGHT NOW, determine the single most valuable next action, decide whether FounderOS itself can perform it, and only ask the founder to do the part it genuinely can't. You may propose up to 3 candidate actions, but each one must independently be a genuinely defensible "most valuable next action," not padding.

FounderOS's ONLY real executable capability right now is live web research and analysis (searching the web, reading pages, comparing sources, reasoning over what's already known/stored). It cannot send emails, publish anything, contact anyone, or take any other external action — there is no tool for that yet. So classify every candidate's actionType honestly:
- "AUTONOMOUS": answerable through research/analysis alone — e.g. "who are the competitors," "is there public demand signal," "what's the typical pricing in this space," "what do reviews/discussions say about X." Set needsResearch: true and give a clear researchTopic — FounderOS will actually run this research itself; the founder never sees it as a task at all.
- "APPROVAL_REQUIRED": FounderOS would need to take an external action with real consequences (sending something, publishing something, spending money) — but no such tool currently exists in this application, so do NOT propose one of these unless you are certain it maps to a capability that's actually described in your available tools. When genuinely unsure, classify as FOUNDER_REQUIRED instead — never invent a capability FounderOS doesn't have.
- "FOUNDER_REQUIRED": genuinely requires the founder — a real conversation, a physical action, something only they can authorize or do. For these, set needsResearch: true and researchTopic whenever research could still do useful prep first (e.g. before "interview 5 customers," research the target segment and draft the questions) — FounderOS will run that research and fold the real results into the task before showing it, so the founder only ever does the part that's truly left. If no research would genuinely help, set needsResearch: false.

How to reason, in order:
1. Identify the current bottleneck (founderState.currentBottleneck, already synthesized) — the action should normally address THAT, not a generic stage playbook.
2. Weigh recentChanges more heavily than "time has passed." Time passing alone is never a reason to propose an action.
3. Build on recentlyCompletedWithOutcomes — including prior research findings, which appear there the same way task discoveries do. When a finding implies a clear next step, propose THAT.
4. Ground every proposal in real evidence — never propose something because it's a thing startups are generally supposed to do.
5. Be specific to THIS startup, never generic.
6. Review completedTasks and blockedTasks before proposing anything.
7. Do NOT repeat a previous action, or one substantially equivalent to one already completed (including a prior research pass on the same topic), unless founderState/recentChanges shows a clear reason the previous result is stale or no longer sufficient.
8. If excludedObjectiveKeys is non-empty, those were just rejected by the backend as duplicates with no justified reason to revisit — find something genuinely different.
9. If an action genuinely depends on another not-yet-completed one, say so via dependsOnTitle.
10. Never invent an assumption to fill a real gap in founderState — work with what's actually known.
...
Output ONLY JSON:
{
  "tasks": [
    {
      ...
      "objective": "1 sentence — what this action is actually for",
      "objectiveKey": "a short, stable, lower_snake_case identifier for the underlying goal, independent of wording — e.g. 'validate_customer_problem', 'research_competitors', 'test_pricing_offer'. Two actions with the same underlying goal MUST get the same objectiveKey even if worded differently.",
      "category": one of ${JSON.stringify(TASK_CATEGORIES)},
      "actionType": one of ${JSON.stringify(ACTION_TYPES)},
      "needsResearch": true|false,
      "researchTopic": "if needsResearch is true: a clear, specific research topic/question for FounderOS to investigate (this becomes the input to its own research planner) — grounded in this startup's actual product/customer/positioning, not generic. Null if needsResearch is false.",
      "reason": "the underlying rationale, grounded in specific evidence from founderState/recentlyCompletedWithOutcomes/recentDecisions — not generic startup advice.",
      "whyNow": "1-2 sentences: what's happening RIGHT NOW that makes this the right action at this moment.",
      "expectedOutcome": "1 sentence — what this should concretely reveal or produce if done",
      "contextReferences": ["2-5 short strings naming the SPECIFIC pieces of context that actually drove this decision — never invent one"],
      "dependsOnTitle": "exact title of another task this requires first, or null",
      "steps": [ { "title": "short step name", "instructions": "concrete instructions" } ] (2-5 steps — for a FOUNDER_REQUIRED action, this is what's left for the founder to actually do; for an AUTONOMOUS one, describe what FounderOS itself will do),
      "completionCriteria": "the objective condition for done — doubles as the founder-facing 'success looks like' statement",
      "evidenceRequirements": "what evidence would prove a FOUNDER_REQUIRED action was done; for AUTONOMOUS, describe what a successful research pass looks like",
      "requiredThreshold": number 0.4-0.9 — evidence strength required before counting as verified (FOUNDER_REQUIRED only; irrelevant for AUTONOMOUS, which is verified by whether research actually succeeded),
      "revisitConditions": ["founderState field names that, if changed later, would justify redoing this — see the existing list: founderName, founderRole, founderExperience, founderSkills, founderGoals, startupName, goal, currentMilestone, stage, problem, targetCustomer, currentSolution, businessModel, traction, revenue, competitors, fundingStatus"],
      "priorityFactors": {
        "impact": number 0-1, "urgency": number 0-1, "uncertaintyReduction": number 0-1, "riskReduction": number 0-1, "effort": number 0-1
      }
    }
  ]
}`
  ...
  const userMsg = JSON.stringify({
    founderState,
    currentBottleneck: founderState?.currentBottleneck,
    currentMilestone: founderState?.currentMilestone,
    recentChanges: recentChanges || [],
    completedTasks: (completedTasks || []).map((t) => ({ title: t.title, objective: t.objective, objectiveKey: t.objectiveKey, category: t.category, actionType: t.actionType, completedAt: t.completedAt })),
    recentlyCompletedWithOutcomes: recentlyCompletedWithOutcomes || [],
...
// ---- Live research: planner + synthesis (§7-§10 of the autonomous-
// execution spec) ----
// Two separate AI calls, mirroring webResearchService's own separation of
// concerns: planResearchQueries decides WHAT to search for (before any
// search runs), synthesizeResearch reasons over WHAT CAME BACK (after
// search+fetch actually ran). Neither one calls SearXNG or does any
// fetching itself — executionEngine is the only thing that wires these
// together with webResearchService's real search()/fetchAndExtract()
// calls, so this file never has to fabricate what a search "would have"
// returned.

// §8 — generates the actual queries to run, grounded in the startup's
// real context so results come back startup-specific rather than generic
// (§11/§12). Deliberately capped at a handful of queries — §8's own
// "avoid excessive searches" instruction.
async function planResearchQueries(userId, { topic, startupContext }) {
  const system = `You are planning a live web research pass for FounderOS. Given a research topic/question and this startup's actual context, decide what specific searches would actually establish a reasonable evidence base — not just re-searching the topic sentence verbatim once.

Ground every query in the startup's actual product, target customer, or positioning where relevant — a query that would return the same generic results for any startup in this space is too broad. Use 2-5 queries; more only if the topic genuinely has multiple distinct angles worth checking (e.g. competitors AND pricing AND customer sentiment are three different searches, not one).

Output ONLY JSON:
{ "queries": ["query 1", "query 2", ...] }`;
  const userMsg = JSON.stringify({ topic, startupContext: startupContext || undefined });
  return callAI(system, [{ role: "user", content: userMsg }], {
    json: true,
    shape: { requiredKeys: ["queries"] },
    maxTokens: 300,
  });
}

// §9/§10 — the actual reasoning layer over real, already-fetched sources
// (never invents a source, never fabricates content beyond what's given).
// Fact/inference/uncertainty separation and cross-source comparison are
// both explicit, required output — not left to prose to maybe cover.
async function synthesizeResearch(userId, { topic, startupContext, sources }) {
  const system = withMemoryContext(
    userId,
    `research: ${topic}`,
    `You are analyzing real web research results for FounderOS, to answer a specific research question for this founder's startup. You are given a set of sources that were ACTUALLY searched and fetched — never claim to have read a source that isn't in this list, and never state something as established unless a given source actually supports it.

Distinguish clearly:
- FACT: what a source explicitly states.
- INFERENCE: what can reasonably be concluded by combining multiple pieces of evidence — never present this as fact.
- UNCERTAINTY: what the sources genuinely don't establish either way.

Apply the startup's own context (product, target customer, positioning, stage) — the goal is a startup-specific conclusion, not a generic summary of what's out there (§11).

If sources disagree, say so explicitly rather than picking one silently.

Output ONLY JSON:
{
  "facts": ["short factual statements, each traceable to a specific source given"],
  "inferences": ["short inference statements — clearly conclusions drawn from combining evidence, not direct quotes/facts"],
  "uncertainties": ["short statements of what remains genuinely unclear from these sources"],
  "disagreements": ["if sources conflict: 'Source A says X, Source B says Y' style statements — empty array if none"],
  "conclusion": "2-3 sentences — the actual bottom-line answer to the research topic, applying the startup's specific context, not a generic recap of the sources",
  "recommendation": "1-2 sentences — what this suggests FounderOS/the founder should do next, grounded in the conclusion above",
  "confidence": "low"|"medium"|"high" — based on source quality/quantity/agreement, never inflated for the sake of sounding decisive
}
Never fabricate a source, statistic, or quote that isn't genuinely present in the sources given.`
  );
  const userMsg = JSON.stringify({
    topic,
    startupContext: startupContext || undefined,
    sources: (sources || []).map((s) => ({ title: s.title, url: s.url, snippet: s.snippet, content: s.content ? s.content.slice(0, 3000) : null, fetchFailed: s.fetchFailed || null })),
  });
  return callAI(system, [{ role: "user", content: userMsg }], {
    json: true,
    shape: { requiredKeys: ["conclusion", "confidence"] },
    maxTokens: 900,
  });
}

// §22-24 — composes the founder-facing narration for a turn where
// FounderOS actually executed autonomous work (research) before either
// resolving the objective entirely or arriving at a genuinely
// founder-required residual action. Only ever phrases things that
// actually happened — executedActions/founderRequiredTask are real
// results from webResearchService + the functions above, assembled by
// executionEngine, never invented here. This is the ONE place that turns
// "I did X, found Y, here's what's left" into natural prose instead of a
// rigid template — see §24's suggested shape, used as a loose guide, not
// a forced format for every turn.
async function draftAutonomousExecutionReply(userId, { objective, executedActions, founderRequiredTask, failedActions }) {
  const system = `You are FounderOS, reporting back to a founder after actually doing some work autonomously — not assigning them homework. Write ONE natural, direct message (not a rigid template, not headers for their own sake) that makes clear: what you found, what you did, what it means, and — only if something genuinely couldn't be done autonomously — the one thing left for them, grounded in exactly what's given below.

Rules:
- Only state something as done if it's actually in executedActions. Never claim to have researched/found something that isn't there.
- If failedActions is non-empty, be honest about what didn't work and why — never silently omit a failure or pretend it succeeded.
- If founderRequiredTask is given, introduce it naturally as the one remaining thing, explicitly mentioning what you already did to prepare it (from its steps/autonomousWork) so it reads like real prep, not a bare ask.
- If founderRequiredTask is null and there were no failures, the objective is fully resolved by the autonomous work — say so plainly, no dangling ask.
- Keep it concise — a working update, not an essay. Bullets are fine for multiple findings/actions, but don't force headers onto a short, simple update.
- Never expose raw internal field names, scores, or reasoning steps — write like a sharp cofounder giving a real status update.`;
  const userMsg = JSON.stringify({
    objective,
    executedActions: (executedActions || []).map((a) => ({ topic: a.topic, conclusion: a.conclusion, recommendation: a.recommendation, confidence: a.confidence, sourceCount: (a.sources || []).length })),
    failedActions: (failedActions || []).map((a) => ({ topic: a.topic, reason: a.reason })),
    founderRequiredTask: founderRequiredTask
      ? { title: founderRequiredTask.title, whyNow: founderRequiredTask.whyNow, autonomousWork: founderRequiredTask.autonomousWork, firstStepInstructions: founderRequiredTask.steps?.[0]?.instructions }
      : null,
  });
  return completeAIReply(system, [{ role: "user", content: userMsg }], { maxTokens: 500 });
}

// §6/§7/§13 of the context-aware-task-generation spec — captured once, at
// real completion, from the task's own evidence log and the verification
// that already happened for it. Turns "task: done" into something the
// NEXT proposeNextTasks call can actually build a causal chain on
// (observation → task → result → learning → next task) instead of a flat
// list of completed titles. Deliberately narrow-scoped to just this one
// task's evidence — never invents a discovery the evidence doesn't
// support, and never re-derives founder-wide state (that's
// synthesizeFounderState's job, called separately).
async function synthesizeTaskOutcome(userId, { task, evidenceLog, verificationNotes }) {
  const system = `You are extracting the real, concrete outcome of a just-completed FounderOS task, so future task planning can build on what was actually learned — not just that the task was "done."

Task: "${task.title}"
Objective: ${task.objective}
Completion criteria: ${task.completionCriteria || "not specified"}

All evidence the founder submitted for this task, in order:
${(evidenceLog || []).map((e, i) => `${i + 1}. ${e.text}`).join("\n") || "(none recorded)"}

Verification notes: ${verificationNotes || "(none)"}

Output ONLY JSON:
{
  "summary": "1-2 sentences — what actually happened/was found, in plain terms",
  "discoveries": ["short factual findings actually supported by the evidence above — e.g. '4 of 5 interviewed customers found onboarding confusing at the pricing step'. Empty array if the evidence doesn't support any concrete discovery beyond 'task completed'."],
  "decisions": ["any decision the founder explicitly made or clearly implied based on this task's outcome — empty array if none"],
  "implications": ["1-3 short statements about what this outcome suggests should happen next, or what it changes about the startup's situation — e.g. 'pricing page likely needs to be clarified or moved later in the flow'. Only include implications genuinely supported by the evidence — never speculate beyond it."]
}
Ground everything strictly in the evidence given — never invent a finding, number, or quote that isn't actually there. If the evidence is thin (e.g. a bare claim with no detail), it's fine and correct for discoveries/implications to be empty or minimal.`;
  return callAI(system, [{ role: "user", content: (evidenceLog || []).map((e) => e.text).join("\n\n") || "(no evidence text)" }], {
    json: true,
    shape: { requiredKeys: ["summary"] },
    maxTokens: 500,
  });
}
...
async function classifyExecutionMessage(userId, { task, currentStep, founderMessage, recentHistory, canSearch }) {
...
Output ONLY JSON:
{
  ...
  "needsSearch": true|false — true ONLY if intent is QUESTION or OFF_TOPIC AND answering well genuinely requires current/external information (finding communities, current trends, competitors, documentation, "what are people saying about..." — the kind of thing that needs a real web search, not your own knowledge). ${canSearch ? "" : "Web search is not available right now, so always leave this false and answer from what you already know instead."} False for anything answerable from context or general knowledge that doesn't change.
  "searchQuery": "If needsSearch is true: a good search query for it. Null otherwise.",
  "responseText": "Your reply, ONLY if intent is QUESTION or OFF_TOPIC AND needsSearch is false — actually help them execute, don't just repeat the instructions back. For OFF_TOPIC, answer briefly if it's a real question, but keep it short and bring focus back naturally. Null if intent is STUCK or EVIDENCE, or if needsSearch is true (the reply gets drafted after the search results come back instead).",
...
// Bridges a founder question mid-task into a real, sourced answer. Used
// when classifyExecutionMessage flags needsSearch — deliberately a single
// search-then-answer pass (not a multi-round agentic loop like
// aiService.chat()'s web_search tool) to match the execution engine's own
// per-turn, single-shot-JSON-call architecture rather than bolting a
// different calling convention onto it. Reuses aiTools.executeTool for
// the actual search so there's exactly one place that talks to SearXNG
// and exactly one place that turns its failures into founder-safe text.
async function answerFromSearchResults(userId, { founderMessage, searchResultText, task }) {
  const system = `You are FounderOS, answering a founder's question mid-task using real web search results. Current task for context: "${task.title}".

Search results:
${searchResultText}

Write one natural, direct, conversational reply to the founder's question, grounded in these results. Never state something as a search finding unless it's actually in the results above — if the results don't really answer the question, say so plainly rather than filling the gap with a guess. Don't include raw URLs in your reply text itself (they're shown separately as sources) — just refer to sources naturally in prose if needed (e.g. "a few Reddit threads suggest...").`;
...
async function verifyTaskEvidence(userId, { task, currentStep, evidenceText, priorEvidenceForStep }) {
  const priorBlock = (priorEvidenceForStep || []).length
    ? `\nEarlier evidence already submitted for this same step (consider this cumulative, not a separate attempt):\n${priorEvidenceForStep.map((t, i) => `${i + 1}. ${t}`).join("\n")}\n`
    : "";
...
${priorBlock}
The founder just submitted this as additional evidence for the current step:
"""
${evidenceText}
"""
...
Output ONLY JSON:
{
  "verificationScore": number 0.0-1.0 using this rubric, judged against ALL evidence for this step so far (prior + this new submission combined), not just the newest message in isolation —
    ...
    0.5 = partial evidence — a genuine, credible start, but not yet enough to fully satisfy what this step needs (e.g. 2 of 5 required interviews, described with real detail)
    0.75 = strong evidence — specific, credible, close to or covering the full requirement
  ...
  "notes": "Specific, honest assessment for internal record — what was and wasn't shown, and where things stand cumulatively.",
  "feedbackToFounder": "What to actually say to the founder — write this as one natural, warm, conversational reply, the way a sharp cofounder would respond in a chat, never as a status readout. If this step needs several of something (e.g. 5 interviews) and the founder just gave one more that's genuinely credible, acknowledge that one specifically and naturally mention where the count stands (e.g. 'Good — that's 2 of the 5 so far. What did the next person say?'), then prompt for the next one. If insufficient/a bare claim: be specific and direct about exactly what's missing, never harsh, never fake praise. If the step is now fully satisfied: acknowledge specifically what they showed — don't just say 'great job'."
}
...
// §12 (activity/evidence spec) — decides whether an observed activity
// (a document upload, a decision recorded, etc.) is actually relevant
// evidence for the founder's current task. Deliberately narrow-scoped
// input — just this one task and this one activity, never the whole app
// state — both to keep the call cheap and because that's genuinely all
// the context a relevance judgment needs. Called only for activities
// that didn't already match via the deterministic type-based rules in
// activityEvidenceEngine.js (obvious cases don't need an AI call at all).
async function matchActivityToTask(userId, { task, currentStep, activity }) {
  const system = `You are deciding whether something FounderOS observed happening is actually evidence for a founder's current task — not whether it's impressive, just whether it genuinely relates.

Task: "${task.title}"
What this task actually needs to be considered done: ${task.evidenceRequirements || task.completionCriteria}
Current step: "${currentStep?.title || task.title}" — ${currentStep?.instructions || ""}

Observed activity: ${activity.description}
Activity type: ${activity.type}

Output ONLY JSON:
{
  "relevant": true|false,
  "relevanceScore": number 0-1 — how directly this activity relates to what this task needs (0 = unrelated, 1 = exactly what's needed),
  "strength": "weak"|"medium"|"strong" — how strong this counts as evidence on its own. A system-observed action (e.g. a real upload, a real recorded decision) is inherently stronger than a founder's unverified claim would be, but still weak/medium if it only partially covers what the task needs.
  "reasoning": "one sentence — why this does or doesn't count"
}
Be conservative — an activity that's merely adjacent to the task's topic (e.g. uploaded an unrelated document while this task is about customer interviews) is NOT relevant. Only mark relevant when the activity plausibly demonstrates real progress on THIS specific task.`;
  return callAI(system, [{ role: "user", content: activity.description }], {
    json: true,
    shape: { requiredKeys: ["relevant", "relevanceScore", "strength"] },
    maxTokens: 300,
  });
}
...
module.exports = {
  ...
  synthesizeOnboardingTurn,
  synthesizeFounderState,
  proposeNextTasks,
  synthesizeTaskOutcome,
  planResearchQueries,
  synthesizeResearch,
  draftAutonomousExecutionReply,
  TASK_CATEGORIES,
  ACTION_TYPES,
...
async function proposeNextTasks(
  ...
  const system = withMemoryContext(
    userId,
    "execution task generation",
    `You are the task planner for FounderOS. Your job is NOT "generate a startup task" — it is: given everything currently known about THIS founder and THIS startup RIGHT NOW, determine the single most valuable next action, and be able to explain why it's the right action now specifically. You may propose up to 3 candidates, but each one must independently be a genuinely defensible "most valuable next action," not padding — and two founders at the same nominal stage but different actual situations (different evidence, different bottleneck, different recent results) should get different tasks. If your proposal would look the same for most startups at this stage regardless of their specific evidence, it's too generic — go back and ground it in what THIS founder's context actually shows.

How to reason, in order:
1. Identify the current bottleneck (founderState.currentBottleneck, already synthesized) — the task should normally address THAT, not a generic stage playbook. A founder with users but no revenue needs a monetization task, not another acquisition task; a founder with signups but no activation needs an activation task, not another feature.
2. Weigh recentChanges (what's actually different since last time — a changed target customer, a failed experiment, new feedback, a metric that moved) more heavily than "time has passed." Time passing alone is never a reason to propose a task — something must have changed, resolved, or newly come to light.
3. Build on recentlyCompletedWithOutcomes: their discoveries/decisions/implications are real findings, not just "task done" checkmarks. When a finding implies a clear next step, propose THAT — this is how a chain like "interviewed 5 customers → found onboarding confusion → observe onboarding directly → found the exact step → fix that step" actually forms, instead of disconnected tasks.
4. Ground every proposal in real evidence (founderState, recentlyCompletedWithOutcomes, recentDecisions) — never propose a task because it's a thing startups are generally supposed to do. A task justified only by category ("you should validate your idea") is worse than one justified by a specific fact ("3 of your last 5 signups mentioned pricing confusion").
5. Be specific to THIS startup, never generic. "Talk to customers" is not a task; "interview 5 users who abandoned onboarding this week and identify the step where they stopped" is. Transform every generic-sounding action into one anchored in this founder's actual customers, product, numbers, or stated problem.
6. Review completedTasks and blockedTasks before proposing anything — don't propose a duplicate (handled deterministically by the backend, but you should already be avoiding it) and don't propose something that's actually blocked on a dependency that isn't done yet.
7. Do NOT repeat a previous task, or a task substantially equivalent to one already completed, unless founderState/recentChanges shows a clear reason the previous result is no longer sufficient (e.g. targetCustomer, problem, currentSolution, businessModel, or stage materially changed since that task's completedAt). If you do revisit one, say so explicitly in reason (what changed and why the old result no longer holds).
8. If excludedObjectiveKeys is non-empty, those specific objectives were just rejected by the backend as duplicates with no justified reason to revisit — find a genuinely different next action instead.
9. If a task genuinely depends on another not-yet-completed task (including one of the ones you're proposing now), say so via dependsOnTitle.
10. If founderState genuinely doesn't contain enough to ground a real decision on some specific point, don't invent an assumption to fill the gap — work with what's actually known rather than guessing.

Output ONLY JSON:
{
  "tasks": [
    {
      "title": "short, specific, e.g. 'Interview 5 users who abandoned onboarding this week'",
      "objective": "1 sentence — what this task is actually for",
      "objectiveKey": "a short, stable, lower_snake_case identifier for the underlying goal, independent of wording — e.g. 'validate_customer_problem', 'diagnose_onboarding_drop', 'test_pricing_offer'. Two tasks with the same underlying goal MUST get the same objectiveKey even if you'd phrase the title differently.",
      "category": one of ${JSON.stringify(TASK_CATEGORIES)},
      "reason": "the underlying rationale for this task, grounded in specific evidence from founderState/recentlyCompletedWithOutcomes/recentDecisions — not generic startup advice. This is the 'what evidence points here' explanation.",
      "whyNow": "1-2 sentences: what's happening RIGHT NOW — which specific recent change, finding, or bottleneck — makes this the right action at this moment rather than something else. E.g. '4 of your last 6 new users failed to complete onboarding, and your last interview round pointed at pricing confusion, so we need direct observation before changing anything.'",
      "expectedOutcome": "1 sentence — what this should concretely reveal or produce if done (e.g. 'a clear picture of the specific onboarding step causing drop-off')",
      "contextReferences": ["2-5 short strings naming the SPECIFIC pieces of context that actually drove this decision — a stat, a stated goal, a prior finding, e.g. '4 of 6 recent users abandoned onboarding', 'previous interview task found pricing confusion'. Every entry must trace to something actually present in founderState/recentlyCompletedWithOutcomes/recentDecisions/recentChanges — never invent one."],
      ...
      "completionCriteria": "the objective condition that must be true for this task to count as done — this doubles as the founder-facing 'success looks like' statement",
      ...
      "revisitConditions": ["founderState field names — from: founderName, founderRole, founderExperience, founderSkills, founderGoals, startupName, goal, currentMilestone, stage, problem, targetCustomer, currentSolution, businessModel, traction, revenue, competitors, fundingStatus — that, if they change AFTER this task completes, would genuinely justify redoing it. Empty array only if truly nothing about this task's validity depends on any tracked field changing."],
      "priorityFactors": {
  ...
  const userMsg = JSON.stringify({
    ...
    currentMilestone: founderState?.currentMilestone,
    recentChanges: recentChanges || [],
    completedTasks: (completedTasks || []).map((t) => ({ title: t.title, objective: t.objective, objectiveKey: t.objectiveKey, category: t.category, completedAt: t.completedAt })),
    recentlyCompletedWithOutcomes: recentlyCompletedWithOutcomes || [],
    blockedTasks: blockedTasks || [],
    recentDecisions: recentDecisions || [],
    currentlyActiveOrAvailable: activeTaskTitles || [],
...
// different calling convention onto it. Reuses aiTools.executeTool for
// the actual search so there's exactly one place that talks to SearXNG
// and exactly one place that turns its errors into founder-safe text.
async function answerFromSearchResults(userId, { founderMessage, searchResultText, task }) {
  const system = `You are FounderOS, answering a founder's question mid-task using real web search results. Current task for context: "${task.title}".
