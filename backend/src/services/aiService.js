...
// startup health score into one mentor-style briefing. This V2 app has no
// `profile`/`businessState`/`missions` concepts — only companyProfile,
// metrics, and decisions — so this reads from exactly what's actually
// available on the Simulator screen. Reasons about causes and next
// actions rather than restating numbers already shown elsewhere. Never
// invents data it wasn't given — anything unknown goes in missingInfo.
async function generateFounderAdvisor(userId, { companyProfile, metrics, decisions, healthLatest }) {
  const system = withMemoryContext(
    userId,
    "AI Founder Advisor briefing",
    `You are the AI Founder Advisor inside FounderOS — reason like an experienced startup mentor who has just reviewed this founder's full picture (company profile, current metrics, decision/simulation history, and the latest startup health check). Give specific, non-generic recommendations that would differ for a different founder in a different situation. Never repeat a metric back as if that were insight — explain the underlying WHY. Never invent data you weren't given; if something important is unknown, name it in missingInfo instead of guessing. Output ONLY JSON in this exact shape:
{
  "topPriorities": [
  ...
  "rootCauseAnalysis": [
    { "problem": "name of the underperforming metric or area", "likelyCauses": [2-4 short strings reasoning about WHY, not restating the number] }
  ] (0-3 items; omit entirely if nothing is clearly underperforming),
  "actionPlan": {
...
Base every claim strictly on the evidence given below. Adjust tone and substance to this founder's actual stage, numbers, and history — never output advice that would apply equally to any startup. If metrics are mostly null/Unknown, say so plainly in confidenceReasoning and lean on missingInfo rather than inventing numbers.`
  );

  const userMsg = JSON.stringify({
    companyProfile,
    currentMetrics: metrics,
    recentDecisions: (decisions || []).slice(-6).map((d) => ({
      decisionText: d.decisionText,
      status: d.status,
      overallConfidence: d.overallConfidence,
      mainRisks: d.mainRisks,
      accuracySummary: d.accuracyResults?.length
        ? d.accuracyResults.map((a) => `${a.metric}: predicted ~${a.predictedMid}, actual ${a.actual}`).join("; ")
        : undefined,
      pastLearning: d.learningSummary
